  ...
  module V1
    class AlertsController < ApplicationController
      def create
        ...
        # Check if the user already has an active alert for the same cryptocurrency
        ...
        if existing_alert
          render json: { error: 'User already has an active alert for this cryptocurrency' },
                 status: :unprocessable_entity
          return
        end
        # Check if the target price is greater than the current price
        ...
        if current_price && @alert.target_price <= current_price
          render json: { error: 'Target price must be greater than the current price' }, status: :unprocessable_entity
          return
        end
